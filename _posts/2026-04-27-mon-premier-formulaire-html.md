---
title: "Mon premier formulaire HTML"
date: 2026-04-27 09:45:00 +0200
categories: formation
groupe: astrolab
author: Laurent Staelens
image: /assets/images/astrolabe/MonPremierFormulaireVignette.png
description: "Consigne courte du projet formulaire HTML avec génération d'un fichier CSV."
---

![Banderole Mon premier formulaire](/assets/images/astrolabe/MonPremierFormulaireBanderole.png)

Le groupe Astrolabe travaille sur un nouveau projet : créer un formulaire HTML capable de récupérer les réponses d'un utilisateur et de générer un fichier CSV.

L'objectif est de réutiliser les bases du HTML, puis d'ajouter un peu de JavaScript pour rendre le formulaire fonctionnel.

## Consigne courte

Créer une page `index.html` contenant :

- un titre ;
- un texte d'explication ;
- un formulaire fonctionnel ;
- un champ texte pour le nom ou le prénom ;
- une question à choix unique avec des boutons radio ;
- une question à choix multiple avec des cases à cocher ;
- une liste déroulante ;
- une zone de commentaire ;
- un bouton d'envoi.

Le formulaire doit utiliser JavaScript pour récupérer les réponses, créer un fichier CSV et déclencher son téléchargement.

Le thème est libre : habitudes numériques, loisirs, préférences, musique, jeux, films ou utilisation d'Internet.

## Niveaux possibles

- Débutant : formulaire fonctionnel et CSV généré.
- Intermédiaire : ajouter une question, plusieurs choix et une meilleure mise en forme.
- Avancé : ajouter une validation, un message de remerciement et stocker plusieurs réponses avec `localStorage`.

## Suivi des projets

| Prénom | Validé (Oui / Non) | Voir projet |
| --- | --- | --- |
| Oumou-Koulthoum | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Ouzefa | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Sherine | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Maryam | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Nils | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Adama | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Gname | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Rayan | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| William | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Lohan | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Mohamed-Zineddine | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Esteban | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Moise | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Hermell | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Amjad | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Jess-Bruno | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Mehdi | ☐ Oui / ☐ Non | [Lien à compléter](#) |
| Bogdan-Ioan | ☐ Oui / ☐ Non | [Lien à compléter](#) |

![Pied de page Mon premier formulaire](/assets/images/astrolabe/MonPremierFormulairePiedDePage.png)
